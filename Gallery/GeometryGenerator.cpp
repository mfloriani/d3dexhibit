...
void GeometryGenerator::CreateBox(const float width, const float height, const float depth, uint32_t numSubdivisions, MeshData& meshData)
{
...
void GeometryGenerator::CreateSphere(const float radius, const uint32_t sliceCount, const uint32_t stackCount, MeshData& meshData)
{
	...
	for (uint32_t i = 1; i <= stackCount - 1; ++i)
	{
		...
		for (uint32_t j = 0; j <= sliceCount; ++j)
		{
			...
			const XMVECTOR T = XMLoadFloat3(&v.TangentU);
			XMStoreFloat3(&v.TangentU, XMVector3Normalize(T));

			const XMVECTOR p = XMLoadFloat3(&v.Position);
...
void GeometryGenerator::Subdivide(MeshData& meshData)
{
	...
	const uint32_t numTris = static_cast<uint32_t>(inputCopy.Indices.size()) / 3;
...
Vertex GeometryGenerator::MidPoint(const Vertex& v0, const Vertex& v1)
{
	...
	const XMVECTOR tex0 = XMLoadFloat2(&v0.TexCoord);
	const XMVECTOR tex1 = XMLoadFloat2(&v1.TexCoord);
...
void GeometryGenerator::CreateQuad(const float x, const float y, const float w, const float h, const float depth, MeshData& meshData)
{
